...
import { Button, Card, DatePicker, Empty, Form, InputNumber, Modal, Popconfirm, Select, Statistic, Table, Tag, Typography } from 'antd';
...
export function PaymentsPage() {
  ...
  const total = useMemo(() => payments.reduce((sum, payment) => sum + (payment.type === 'Возврат' ? -payment.amount : payment.amount), 0), [payments]);
  ...
  const orderLabel = (orderId: string) => { const order = orders.find((item) => item.id === orderId); return order ? `Заказ №${order.number}` : 'Заказ удален'; };
  ...
  return <>
    <Header><div><Typography.Title level={2}>Платежи</Typography.Title><Typography.Text type="secondary">История оплат и возвратов по заказам</Typography.Text></div><Button type="primary" icon={<PlusOutlined />} onClick={openModal}>Добавить платеж</Button></Header>
    <Summary><Card><Statistic title="Всего получено" value={total} suffix="₽" /></Card><Card><Statistic title="Платежей" value={payments.length} /></Card><Card><Statistic title="Возвратов" value={payments.filter((payment) => payment.type === 'Возврат').length} /></Card></Summary>
    <Card><Table<Payment> rowKey="id" dataSource={[...payments].sort((a, b) => b.date.localeCompare(a.date))} locale={{ emptyText: <Empty description="Платежей пока нет" /> }} columns={[{ title: 'Дата', dataIndex: 'date', render: (date: string) => dayjs(date).format('DD.MM.YYYY HH:mm') }, { title: 'Заказ', dataIndex: 'orderId', render: (orderId: string) => orderLabel(orderId) }, { title: 'Тип', dataIndex: 'type', render: (type: PaymentType) => <Tag color={type === 'Возврат' ? 'red' : type === 'Предоплата' ? 'gold' : 'green'}>{type}</Tag> }, { title: 'Сумма', dataIndex: 'amount', render: (amount: number, payment: Payment) => `${payment.type === 'Возврат' ? '-' : '+'}${amount.toLocaleString('ru-RU')} ₽` }, { title: 'Действия', width: 100, render: (_value: string, payment: Payment) => <Popconfirm title="Удалить платеж?" okText="Удалить" cancelText="Отмена" onConfirm={() => deletePayment(payment.id)}><Button danger type="text" icon={<DeleteOutlined />} /></Popconfirm> }]} /></Card>
    <Modal title="Новый платеж" open={isModalOpen} okText="Сохранить" cancelText="Отмена" onCancel={() => setIsModalOpen(false)} onOk={() => form.submit()}><Form form={form} layout="vertical" onFinish={savePayment}><Form.Item name="orderId" label="Заказ" rules={[{ required: true, message: 'Выберите заказ' }]}><Select options={orders.map((order) => ({ value: order.id, label: `Заказ №${order.number} — ${orderTotal(order.id).toLocaleString('ru-RU')} ₽` }))} /></Form.Item><Form.Item name="type" label="Тип платежа" initialValue={paymentTypes[0]} rules={[{ required: true }]}><Select options={paymentTypes.map((type) => ({ value: type, label: type }))} /></Form.Item><Form.Item name="date" label="Дата" rules={[{ required: true, message: 'Выберите дату' }]}><DatePicker showTime format="DD.MM.YYYY HH:mm" /></Form.Item><Form.Item name="amount" label="Сумма" rules={[{ required: true, message: 'Введите сумму' }]}><InputNumber min={0.01} precision={2} /></Form.Item></Form></Modal>
  </>;
}
