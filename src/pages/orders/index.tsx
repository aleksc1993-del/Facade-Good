...
export function OrdersPage() {
  ...
  return <>
    <Filters>
      <FilterSelect allowClear placeholder="Статус" value={statusFilter} options={orderStatuses.map((status) => ({ value: status, label: status }))} onChange={(value) => setStatusFilter(value as OrderStatus | undefined)} />
      <FilterSelect allowClear showSearch optionFilterProp="label" placeholder="Клиент" value={clientFilter} options={clients.map((client) => ({ value: client.id, label: client.name }))} onChange={(value) => setClientFilter(value as string | undefined)} />
