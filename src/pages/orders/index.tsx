...
const itemTotal = (item?: Partial<OrderItem>) => (item?.quantity ?? 0) * (item?.price ?? 0);
...
export function OrdersPage() {
  ...
  const orderTotal = items.reduce((total, item) => total + itemTotal(item), 0);
  ...
  return <>
    ...
    <Card><Table<Order> rowKey="id" dataSource={orders} locale={{ emptyText: <Empty description="Заказов пока нет" /> }} columns={[{ title: 'Номер', dataIndex: 'number' }, { title: 'Клиент', render: (_value: string, order: Order) => clientName(order.clientId) }, { title: 'Статус', dataIndex: 'status', render: (status: OrderStatus) => <Tag>{status}</Tag> }, { title: 'Срок', dataIndex: 'deadline', render: (deadline: string) => deadline ? dayjs(deadline).format('DD.MM.YYYY') : '—' }, { title: 'Комментарий', dataIndex: 'comment', ellipsis: true }, { title: 'Действия', width: 120, render: (_value: string, order: Order) => <Space><Button aria-label="Редактировать" icon={<EditOutlined />} onClick={() => openEditModal(order)} /><Popconfirm title="Удалить заказ?" okText="Удалить" cancelText="Отмена" onConfirm={() => deleteOrder(order.id)}><Button danger aria-label="Удалить" icon={<DeleteOutlined />} /></Popconfirm></Space> }]} /></Card>
    <Modal title={editingOrder ? 'Редактирование заказа' : 'Новый заказ'} open={isModalOpen} okText="Сохранить" cancelText="Отмена" onCancel={closeModal} onOk={() => form.submit()} width={1000}><Form form={form} layout="vertical" initialValues={emptyOrder} onFinish={saveOrder}><FormGrid><Form.Item name="number" label="Номер заказа" rules={[{ required: true, message: 'Введите номер заказа' }]}><Input /></Form.Item><Form.Item name="clientId" label="Клиент" rules={[{ required: true, message: 'Выберите клиента' }]}><Select options={clients.map((client) => ({ value: client.id, label: `${client.name} — ${client.phone}` }))} /></Form.Item><Form.Item name="status" label="Статус"><Select options={orderStatuses.map((status) => ({ value: status, label: status }))} /></Form.Item><Form.Item name="deadline" label="Срок готовности"><DatePicker format="DD.MM.YYYY" style={{ width: '100%' }} /></Form.Item><FullWidthItem name="comment" label="Комментарий"><Input.TextArea rows={3} /></FullWidthItem><ItemsSection><Typography.Title level={4}>Изделия</Typography.Title><Form.List name="items">{(fields, { add, remove }) => <><Table rowKey="key" pagination={false} dataSource={fields} columns={[{ title: 'Название', render: (_value: unknown, field) => <Form.Item name={[field.name, 'name']} rules={[{ required: true, message: 'Введите название' }]}><Input /></Form.Item> }, ...(['height', 'width', 'quantity', 'price'] as const).map((name) => ({ title: name === 'height' ? 'Высота' : name === 'width' ? 'Ширина' : name === 'quantity' ? 'Количество' : 'Цена', render: (_value: unknown, field: { name: number }) => <Form.Item name={[field.name, name]} rules={[{ required: true, message: 'Заполните поле' }]}><InputNumber min={0} style={{ width: '100%' }} /></Form.Item> })), { title: 'Сумма', render: (_value: unknown, field: { name: number }) => <Typography.Text>{itemTotal(items[field.name]).toLocaleString('ru-RU')} ₽</Typography.Text> }, { title: '', render: (_value: unknown, field: { name: number }) => <Button danger type="text" icon={<DeleteOutlined />} onClick={() => remove(field.name)} /> }]} /><Button type="dashed" block icon={<PlusOutlined />} onClick={() => add({ id: crypto.randomUUID(), name: '', height: 0, width: 0, quantity: 1, price: 0 })}>Добавить изделие</Button></>}</Form.List><TotalRow><Typography.Text strong>Стоимость заказа: {orderTotal.toLocaleString('ru-RU')} ₽</Typography.Text></TotalRow></ItemsSection></FormGrid></Form></Modal>
  </>;
}
